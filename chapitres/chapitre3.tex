...
\label{sec:introduction-ch3}
  ...
  spécifiques aux architectures microservices. Pour chacune des six catégories
  de menaces, les contre-mesures apportées par le Service Mesh sont détaillées
  et mises en correspondance avec les mécanismes Istio déployés.
...
correspondance directe avec les mécanismes de protection qu'offre Istio.

  STRIDE est un acronyme développé par Microsoft qui catégorise les menaces en
  six familles : \textbf{S}poofing (usurpation d'identité), \textbf{T}ampering
  (altération de données), \textbf{R}epudiation (non-traçabilité des actions),
  \textbf{I}nformation Disclosure (divulgation d'informations), \textbf{D}enial
  of Service (déni de service) et \textbf{E}levation of Privilege (élévation de
...
Le système étudié est un prototype de système de paiement déployé sur Kubernetes
avec deux microservices communicants :
...
\begin{enumerate}
  ...
  \item Les tentatives d'accès depuis des \textbf{namespaces ou pods non autorisés}.
\end{enumerate}
...
Istio répond à cette menace via deux mécanismes complémentaires :
...
\begin{enumerate}
  \item Le \textbf{mTLS en mode STRICT} impose que chaque service présente un
    ...
    sont générés automatiquement par \texttt{istiod} (l'autorité de certification
    Istio) et encodent l'identité SPIFFE du workload sous la forme :\\
    \texttt{spiffe://cluster.local/ns/\{namespace\}/sa/\{serviceaccount\}}.
...
Toute modification des données chiffrées invalide le \textit{Message Authentication
Code} (MAC) et provoque le rejet de la connexion. Par ailleurs, la validation JWT
assure l'intégrité des tokens d'authentification : un JWT dont la signature a été
modifiée est rejeté avec un code \texttt{401} par le composant
...
manipulable par les applications. Aucune action dans le mesh ne peut être
effectuée sans laisser une trace dans les logs Envoy.

\subsection{Information Disclosure: Divulgation d'Information}

Sans chiffrement, le trafic inter-services est lisible par n'importe quel processus
ayant accès au réseau du cluster. Une capture de paquets (\texttt{tcpdump},
Wireshark) révèle les données métier en clair : montants de transactions,
identifiants de cartes, résultats d'analyse de fraude.
...
\begin{itemize}
  \item Les \textbf{DestinationRules} permettent de définir des \textit{connection
    pools} (limites de connexions simultanées et de requêtes en attente) au niveau
    du sidecar Envoy, sans dépendance à un service externe.
  ...
  \item Les \textbf{EnvoyFilters} permettent de configurer un mécanisme de
    ...
    les requêtes excédentaires avec un code \texttt{429}.
\end{itemize}
...
les ServiceAccounts SPIFFE et les claims JWT.
  ...
  (code \texttt{200} sur la validation de signature) mais rejeté par
  l'\texttt{AuthorizationPolicy} (code \texttt{403}). Cette séparation entre
  les deux niveaux de contrôle est fondamentale pour une défense en profondeur.
  
\section{Synthèse des Risques et Contre-mesures}
% ─────────────────────────────────────────────────────────────────────────────

Le tableau suivant synthétise les douze scénarios d'attaque analysés et
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    T11 & DoS        & Rate limiting EnvoyFilter
        & EnvoyFilter token bucket 50/min
        & \textbf{PROTÉGÉ} \checkmark & 429 \\
    \rowcolor{gray!10}
    T12 & Elevation  & JWT valide, claims insuffisants
        & AuthorizationPolicy claims check
        & \textbf{BLOQUÉ} \checkmark & 403 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
+ SPIFFE + NetworkPolicy} crée une défense en profondeur où chaque couche compense
les limitations des autres. Sur les douze scénarios expérimentés, dix sont
intégralement bloqués, un est tracé et un est partiellement atténué, confirmant
la robustesse de l'architecture déployée.
