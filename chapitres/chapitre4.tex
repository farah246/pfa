...
\begin{itemize}
  \item \textbf{Catégorie STRIDE :} Repudiation
  \item \textbf{Question :} Toutes les requêtes --- légitimes et attaques --- sont-elles
    tracées dans l'audit trail Envoy\,?
  \item \textbf{Résultat attendu :} Les logs Envoy enregistrent chaque requête avec
    méthode, chemin, code HTTP et raison de rejet.
\end{itemize}

\paragraph{Procédure.}
Un port-forward est ouvert sur l'ingress gateway, puis quatre requêtes sont émises :
un \texttt{GET /health} légitime suivi de trois \texttt{POST /charge} sans JWT.
Les logs du sidecar \texttt{istio-proxy} sont ensuite extraits :
...
Des logs additionnels issus de tests JWT montrent deux sous-types de rejet 401 :
...
\begin{table}[H]
  \centering
  \caption{Audit trail T7 --- requêtes enregistrées par Envoy}
  \label{tab:t7-audit}
  \renewcommand{\arraystretch}{1.3}
  \begin{tabularx}{\textwidth}{l l l X}
    \toprule
    \rowcolor{gray!15}
    Requête & Code & Raison Envoy & Interprétation \\
    \midrule
    \texttt{GET /health}   & 200 & \texttt{via\_upstream}
      & Accès légitime, transmis au service \\
    \rowcolor{gray!10}
    \texttt{POST /charge} (×3) & 403 & \texttt{rbac\_access\_denied\_matched\_policy[none]}
      & Rejet RBAC --- aucun JWT présenté \\
    \texttt{POST /charge}  & 401 & \texttt{jwt\_authn\_access\_denied\{Jwt\_issuer\_is\_not\_configured\}}
      & Rejet JWT --- issuer inconnu \\
    \rowcolor{gray!10}
    \texttt{POST /charge}  & 401 & \texttt{jwt\_authn\_access\_denied\{Jwt\_signature\_invalid\}}
      & Rejet JWT --- signature Base64url invalide \\
    \bottomrule
  \end{tabularx}
\end{table}

\paragraph{Ce que ce test prouve.}
Chaque entrée Envoy fournit les cinq dimensions de la non-répudiation :
\textbf{qui} (IP source \texttt{10.244.0.109}, user-agent \texttt{curl/7.81.0}),
\textbf{quoi} (méthode + chemin),
\textbf{quand} (horodatage UTC à la milliseconde),
\textbf{résultat} (200 autorisé / 401--403 rejeté) et
\textbf{pourquoi} (raison machine : \texttt{rbac\_access\_denied} ou
\texttt{jwt\_authn\_access\_denied} avec sous-type).
Aucune requête ne peut traverser le mesh sans être enregistrée par le proxy Envoy :
l'audit trail est exhaustif et ne repose pas sur la coopération de l'application.
