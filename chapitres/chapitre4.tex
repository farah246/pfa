...
paiement déployé sur Kubernetes avec Istio~[11,1], et documente la validation
...
Le prototype est déployé sur Minikube, une distribution Kubernetes locale~[11]
...
\begin{itemize}
  ...
  \item \textbf{Tests :} \texttt{kubectl}, \texttt{curl}, OWASP ZAP~[3],
  ...
  \begin{tabularx}{\textwidth}{
    ...
    fraud-detection
      ...
      & Service interne. Analyse le risque des transactions. Accessible uniquement
        depuis le SA \texttt{payment-api}~[7]. \\
    test-with-sidecar
      & curlimages/curl & ---
...
\begin{table}[H]
  \centering
  \caption{Les sept couches de sécurité du prototype}
  ...
  \begin{tabularx}{\textwidth}{
    ...
    7 & Rate Limiting (EnvoyFilter)
      & Token bucket local --- 20\,req/s par replica
      & T10, T11 --- Flood, DoS \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\paragraph{Commande exécutée.}
\begin{verbatim}
GATEWAY_IP=$(kubectl get svc -n istio-system istio-ingressgateway -o jsonpath='{.spec.clusterIP}')
JWT=$(bash scripts/generate-jwt-rs256.sh user123 customer)
TAMPERED="${JWT%.*}.badsignature"

kubectl exec test-pod -- curl -s -o /dev/null -w "HTTP:%{http_code}\n" \
	-X POST "http://$GATEWAY_IP/charge" \
	-H "Content-Type: application/json" \
	-H "Authorization: Bearer $TAMPERED" \
	-d '{"amount": 100, "card_id": "TEST"}'
\end{verbatim}
...
\caption{Flux JWT à l'Istio Ingress Gateway --- validation et contrôle d'accès.}\label{fig:cycle-vie-certificats}
\end{figure}
\noindent
La requête entrante est d'abord soumise à la \texttt{RequestAuthentication} qui
vérifie la signature RS256, l'issuer et l'expiration du token. En cas de succès,
l'\texttt{AuthorizationPolicy} évalue les claims (rôle, principal SPIFFE) avant
d'autoriser ou rejeter l'accès à \texttt{payment-api}.
...
Ce test valide que toutes les requêtes --- légitimes et tentatives d'attaque ---
sont enregistrées dans les logs d'accès Envoy, garantissant la non-répudiation
au niveau du mesh.

\paragraph{Commandes exécutées.}
\begin{verbatim}
POD=$(kubectl get pods -n payment-system -l app=payment-api \
      -o jsonpath='{.items[0].metadata.name}')

# Requête légitime
curl -s http://localhost:8080/health

# Tentatives sans JWT (x3)
for i in {1..3}; do
  curl -s -X POST http://localhost:8080/charge \
    -H "Content-Type: application/json" \
    -d '{"amount": 100}'
  sleep 0.5
done

# Extraction des logs Envoy
kubectl logs -n payment-system $POD -c istio-proxy --tail=30
\end{verbatim}

\paragraph{Résultat obtenu.}
\begin{verbatim}
[2026-05-03T21:52:55.828Z] "GET /health HTTP/1.1" 200 - via_upstream
  "10.244.0.109" "curl/7.81.0" "2cc2d049-fda3-91aa-ae79-7974eedac9d1"

[2026-05-03T21:54:33.433Z] "POST /charge HTTP/1.1" 403 -
  rbac_access_denied_matched_policy[none]
  "10.244.0.109" "curl/7.81.0" "68186ae7-20f4-9b33-80bd-14ed4829bbb6"

[2026-05-03T21:54:34.031Z] "POST /charge HTTP/1.1" 403 -
  rbac_access_denied_matched_policy[none]
  "10.244.0.109" "curl/7.81.0" "1fc92df8-e0fa-95f3-acc6-31259828ad9e"

[2026-05-03T21:54:34.555Z] "POST /charge HTTP/1.1" 403 -
  rbac_access_denied_matched_policy[none]
  "10.244.0.109" "curl/7.81.0" "3e84b97d-ba67-907b-99ed-b94534759f5d"
\end{verbatim}

Les logs issus des tests JWT (T5--T6) complètent l'audit trail avec deux
sous-types de rejet 401 :

\begin{verbatim}
[2026-05-01T10:45:07.227Z] "POST /charge HTTP/1.1" 401 -
  jwt_authn_access_denied{Jwt_issuer_is_not_configured}

[2026-05-01T10:45:09.501Z] "POST /charge HTTP/1.1" 401 -
  jwt_authn_access_denied{Jwt_signature_is_an_invalid_Base64url_encoded}
\end{verbatim}

Le proxy Envoy enregistre chaque requête avec horodatage UTC à la milliseconde,
méthode, chemin, code de réponse, raison de rejet, IP source, user-agent et
identifiant de trace unique. Ces cinq dimensions --- \textbf{qui}, \textbf{quoi},
\textbf{quand}, \textbf{résultat} et \textbf{pourquoi} --- constituent un audit
trail complet qui ne repose pas sur la coopération de l'application : toute
requête traversant le mesh est nécessairement journalisée par le sidecar,
que la réponse soit un 200, un 401 ou un 403.
...
\subsection{T9 --- Abus de Jeton ServiceAccount (Information Disclosure / Elevation of Privilege)}

Ce test vérifie qu'un pod compromis ne peut pas utiliser le jeton ServiceAccount
monté dans son système de fichiers pour escalader ses privilèges via l'API
Kubernetes.

\paragraph{Commandes exécutées.}
\begin{verbatim}
# Lancement d'un pod éphémère dans le namespace cible
kubectl run -n payment-system test-sa --image=alpine:3.19 -it --rm -- sh

# Lecture du jeton SA monté (simule la compromission du pod)
cat /var/run/secrets/kubernetes.io/serviceaccount/token | head -c 80

# Vérification des permissions RBAC associées au ServiceAccount
kubectl auth can-i create pods -n payment-system \
  --as=system:serviceaccount:payment-system:default
kubectl auth can-i delete pods -n payment-system \
  --as=system:serviceaccount:payment-system:default
kubectl auth can-i patch deployments -n payment-system \
  --as=system:serviceaccount:payment-system:default

# Tentative d'appel direct à l'API Kubernetes avec le jeton volé
TOKEN=$(cat /var/run/secrets/kubernetes.io/serviceaccount/token)
APISERVER="https://$KUBERNETES_SERVICE_HOST:$KUBERNETES_SERVICE_PORT"
curl -s -D - --cacert /var/run/secrets/kubernetes.io/serviceaccount/ca.crt \
  -H "Authorization: Bearer $TOKEN" \
  $APISERVER/api/v1/namespaces/payment-system/pods
\end{verbatim}

\paragraph{Résultat obtenu.}
\begin{verbatim}
# T9a — Jeton SA lisible depuis le pod (compromission simulée)
eyJhbGciOiJSUzI1NiIsImtpZCI6Il9vd1FTVzM5bmFsOTNMdGlTcFprcmRWRWFfSXlFM0h1UXRhQ1d2
...(truncated)

# T9b — Permissions RBAC du ServiceAccount default
can-i create pods  -> no
can-i delete pods  -> no
can-i patch deploy -> no

# T9c — Appel API direct avec le jeton volé
HTTP/2 403
{
  "status": "Failure",
  "message": "pods is forbidden: User
    \"system:serviceaccount:payment-system:default\"
    cannot list resource \"pods\" in API group \"\"
    in the namespace \"payment-system\"",
  "reason": "Forbidden",
  "code": 403
}
\end{verbatim}

Le jeton ServiceAccount est bien accessible depuis le système de fichiers du pod
--- la compromission est techniquement possible. Cependant, l'API Kubernetes
applique les règles RBAC indépendamment de la possession du jeton : les trois
opérations sensibles (\texttt{create}, \texttt{delete}, \texttt{patch}) sont
refusées par \texttt{kubectl auth can-i}, et l'appel direct à l'API retourne
\texttt{HTTP 403 Forbidden} avec le message explicite
\textit{cannot list resource ``pods''}.
Le ServiceAccount \texttt{default} du namespace \texttt{payment-system} ne
dispose d'aucun droit sur les ressources Kubernetes : même en cas d'exfiltration
du jeton, aucune escalade de privilèges n'est possible.
Ce résultat confirme la défense en profondeur : Istio contrôle le trafic
inter-services, et le RBAC Kubernetes contrôle l'accès au plan de contrôle ---
les deux couches sont indépendantes et complémentaires.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{pages/fig4.14.png}
  \caption{\textbf{Vue topologique du mesh --- phase baseline (88,38\,\% de succès).}}
  \label{fig:kiali-graph-baseline}
\end{figure}

\noindent
Le namespace \texttt{payment-system} (encadré gris) regroupe \texttt{payment-api},
\texttt{fraud-detection} et l'ensemble des pods de test générés par
\texttt{observe-traffic.sh} (\texttt{attacker-in-mesh}, \texttt{flooder-in-mesh},
\texttt{sa-untrusted}, \texttt{sa-privileged}).
Les arêtes vertes traduisent un trafic majoritairement sain ; la fine arête rouge
depuis \texttt{istio-ingressgateway} correspond aux premières requêtes sans JWT
rejetées par l'AuthorizationPolicy --- les politiques sont actives dès le démarrage
du script.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth]{pages/no-sidecar.png}
  \caption{\textbf{Workload \texttt{attacker-cross-ns} --- sidecar manquant, trafic nul.}}
  \label{fig:kiali-nosidecar}
\end{figure}

\noindent
Kiali signale \textit{Has Missing Sidecar} sur le pod \texttt{attacker-cross-ns}
(namespace \texttt{other-ns}) : aucun proxy Envoy n'est injecté, donc aucun
certificat mTLS ne peut être présenté.
Le panneau Traffic indique zéro requête HTTP, gRPC ou TCP enregistrée --- le proxy
Envoy côté destination rejette silencieusement la connexion faute d'identité valide.
C'est la démonstration directe du \texttt{PeerAuthentication STRICT} : un appelant
extérieur au mesh est isolé sans qu'aucune règle applicative ne soit nécessaire.

% ── Figure 3 ───────────────────────────────────────────────────────────────

\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\linewidth]{pages/spiffe.png}
  \caption{\textbf{Arête HTTP gateway $\rightarrow$ \texttt{payment-api} --- mTLS activé, principals SPIFFE visibles.}}
  \label{fig:kiali-edge-mtls}
\end{figure}

\noindent
Le cadenas \textit{mTLS Enabled} confirme le chiffrement bout-en-bout sur ce lien.
Les deux identités SPIFFE sont affichées explicitement :
\texttt{spiffe://cluster.local/ns/istio-system/sa/istio-i\ldots} pour le gateway
et \texttt{spiffe://cluster.local/ns/payment-system/sa/p\ldots} pour
\texttt{payment-api}.
Le ratio 88,33\,\% succès / 11,67\,\% erreur correspond à une phase mixte :
requêtes légitimes avec JWT \texttt{role=customer} acceptées en 200,
et requêtes sans JWT ou avec \texttt{role=admin} rejetées en 403 par
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{pages/grafana1.png}
  \caption{\textbf{Grafana Istio Service Dashboard --- pic de latence et sources de trafic distinctes à 11:40.}}
  \label{fig:grafana-service-dashboard}
\end{figure}

\noindent
Le panneau \textit{Incoming Requests By Source And Response Code} distingue
clairement deux sources : \texttt{attacker-in-mesh} (200 via mTLS, arête verte)
et \texttt{istio-ingressgateway} (403 via mTLS, arête jaune) --- chaque couleur
correspond à un code HTTP et donc à une politique Istio distincte.
Le pic à 11:40 sur le panneau \textit{Incoming Request Duration} (latence P95
montant à $\sim$2\,s) coïncide avec la phase de flood du script : sans rate
limiting distribué, la latence augmente avant que l'EnvoyFilter local ne prenne
effet.
Le panneau \textit{Incoming Success Rate} montre la chute à 0\,\% pendant la
fenêtre d'attaque puis le retour à 100\,\% une fois le flood terminé.

% ── Figure 5 ───────────────────────────────────────────────────────────────
\paragraph{Istio Mesh Dashboard --- KPI globaux et inventaire des ressources.}
Navigation : Grafana $\rightarrow$ Dashboards $\rightarrow$ Istio $\rightarrow$
Istio Mesh Dashboard.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{pages/grafana.png}
  \caption{\textbf{Grafana Istio Mesh Dashboard --- 8\,ops/s, 1,67\,ops/s en 4xx, inventaire Istio confirmé.}}
  \label{fig:grafana-mesh-dashboard}
\end{figure}

\noindent
Le dashboard confirme l'activité du mesh pendant l'exécution du script :
\textbf{Global Request Volume} à 8,00\,ops/s avec pic visible lors des phases
de flood ; \textbf{Global Success Rate} à NaN (aucune réponse 2xx dans la fenêtre
--- toutes les requêtes actives sont des rejets 4xx) ; \textbf{4xx rate} à
1,67\,ops/s, reflet direct de l'enforcement des AuthorizationPolicies et de la
RequestAuthentication.
Le panneau de ressources fournit également une corroboration chiffrée de la
section~\ref{sec:mesh-config} : 3 VirtualServices, 2 DestinationRules, 1 Gateway,
1 PeerAuthentication, 1 RequestAuthentication, 2 AuthorizationPolicies ---
exactement les objets déployés dans \texttt{mesh-config/}.
...
aux chapitres précédents. Les 11 scénarios d'attaque couvrant les six catégories
...
\texttt{mesh-config}, il est protégé contre 11 catégories d'attaques.