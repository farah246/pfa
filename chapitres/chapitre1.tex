...
et rendent les communications interservices difficiles à sécuriser~[16]~; d'autre
...
communications interservices internes~[5].
...
développement~[14], et de caractériser précisément les limites du DAST
...
DAST automatisé (OWASP ZAP)~[3], couvrant à la fois les flux \emph{north-south}
...
microservices sur un cluster Kubernetes réel (Minikube avec Istio)~[11],
évaluer l'apport concret du Service Mesh face à douze scénarios d'attaque
couvrant les six catégories STRIDE~[4], et mesurer les limites restantes de
l'approche.
  ...
  observabilité~[1]~; \textbf{(2)}~le DAST interne (OWASP ZAP) comme mécanisme
  d'évaluation continue de la posture de sécurité, étendu aux flux
  interservices~[3]~; \textbf{(3)}~la pipeline DevSecOps comme cadre
  d'intégration et d'automatisation de l'ensemble~[14].
...
\begin{enumerate}[leftmargin=*, label=\textbf{\arabic*.}]
  ...
  \item \textbf{Infrastructure as Code (IaC) sécurisée.} La configuration
    ...
    mêmes revues et analyses de sécurité que le code applicatif~[6].
...
d'un système~[2]. Nous présentons ici les quatre approches majeures.
...
dépassements de tampon, ou les mauvaises pratiques cryptographiques~[6].
...
\begin{itemize}[leftmargin=*]
  ...
  \item Complexité accrue dans les architectures microservices, où la
    multitude des points d'entrée démultiplie la surface à couvrir~[16].
\end{itemize}
...
problématique centrale porte sur la sécurisation des communications
interservices dans les architectures microservices~[16], c'est le \textbf{DAST}
qui présente les enjeux les plus critiques et les limites les plus structurantes.
En tant qu'approche \textit{black-box} simulant des attaques réelles depuis
l'extérieur, il est en première ligne face à la complexité des environnements
...
multipliant d'autant les surfaces à tester~[2,16]. Un système composé de
...
pas directement accessibles depuis l'extérieur du cluster~[11,16]. Les outils DAST
...
pipeline, déclenchée à chaque déploiement en environnement de staging~[14]. Des
...
des API (comme 42Crunch, Traceable~AI, ou les extensions REST/gRPC de ZAP~[3])
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Couverture incomplète des flux asynchrones~:} les
    ...
    difficiles à intercepter et à tester dynamiquement~[16].
  ...
  \item \textbf{Complexité de l'environnement de test~:} reproduire
    ...
    coûteux en ressources et en configuration~[11].
\end{itemize}

Ces limites motivent l'exploration d'une approche hybride intégrant le
Service Mesh comme infrastructure de sécurité complémentaire~[5], objet des
chapitres suivants.
...
mettant en évidence leurs complémentarités et leurs limites respectives~[2,6].
